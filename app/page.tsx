      ...
      <main>
        ...
        <section className="hero-bg" id="join">
          <div className="max-w-6xl mx-auto px-6 pt-20 pb-16 sm:pt-28 sm:pb-20">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
              {/* Left */}
              <div>
                <div className="inline-flex items-center gap-2.5 border border-gray-200 bg-white rounded-full px-3.5 py-1.5 mb-10 shadow-sm anim-fade-in">
                  <span className="pulse-dot" />
                  <span className="text-xs font-medium text-[#374151]">Now live &mdash; accepting Waityrs globally</span>
                </div>

                <h1 className="text-[44px] sm:text-[60px] lg:text-[68px] font-extrabold text-[#0A0A0A] leading-[1.02] tracking-tight anim-fade-up delay-1">
                  Something is<br />coming.{' '}
                  <span className="text-[#0D9488]">Get in line.</span>
                </h1>

                <p className="mt-5 text-[17px] sm:text-[19px] text-[#6B7280] max-w-md leading-relaxed anim-fade-up delay-2">
                ...
                <div className="mt-7 anim-fade-up delay-4">
                  <WaitlistForm referralCode={referralCode} />
                </div>
              </div>

              {/* Right — stats */}
              <div className="hidden lg:grid grid-cols-2 gap-4 anim-scale-in delay-3">
                {[
                  { value: stats.total_waiters > 0 ? stats.total_waiters.toLocaleString() : '—', label: 'Waityrs in queue', icon: <Users className="w-4 h-4 text-[#0D9488]" /> },
                  { value: stats.total_revenue_cents > 0 ? `$${(stats.total_revenue_cents / 100).toFixed(0)}` : '$0', label: 'Spent chasing #1', icon: <TrendingUp className="w-4 h-4 text-[#0D9488]" /> },
                  { value: stats.top_spot_record > 0 ? stats.top_spot_record.toString() : '—', label: 'Record #1 reclaims', icon: <Trophy className="w-4 h-4 text-[#0D9488]" /> },
                  { value: '0', label: 'Products shipped', icon: <Zap className="w-4 h-4 text-[#9CA3AF]" /> },
                ].map((s, i) => (
                  <div key={i} className="card-hover border border-gray-200 rounded-xl p-5 bg-white">
                    <div className="w-7 h-7 rounded-lg bg-[#0D9488]/10 flex items-center justify-center mb-3">
                      {s.icon}
                    </div>
                    <p className="text-2xl font-extrabold text-[#0A0A0A] tabular-nums tracking-tight">{s.value}</p>
            ...
            {/* Mobile stats strip */}
            <div className="mt-12 pt-8 border-t border-gray-100 grid grid-cols-2 sm:grid-cols-4 gap-6 lg:hidden anim-fade-up delay-5">
              {[
          ...
          <div className="max-w-6xl mx-auto px-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
              <div>
                ...
                <h2 className="text-2xl sm:text-3xl font-extrabold text-[#0A0A0A] tracking-tight mb-3">
        ...
        <section className="border-t border-gray-100 py-28">
          <div className="max-w-6xl mx-auto px-6 text-center">
            <h2 className="text-3xl sm:text-5xl font-extrabold text-[#0A0A0A] tracking-tight mb-4 leading-tight">
              Get in line.<br />
