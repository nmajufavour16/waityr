...
const MAX_ITEMS = 50;
const STACK_VISIBLE = 4; // how many cards peek in the stack
...
function StackCard({ item, index, total, onDismiss, isAnimatingOut }: StackCardProps) {
  const isTop = index === 0;
  const maxPeek = Math.min(total - 1, STACK_VISIBLE - 1);

  // Each card behind: scale down and translate down slightly
  ...
  const style: React.CSSProperties = isAnimatingOut && isTop
    ...
    : {
        ...
        opacity,
        zIndex,
        transition: isTop ? undefined : 'all 0.35s cubic-bezier(0.22,1,0.36,1)',
      };
  ...
  return (
    <div
      className={`absolute inset-x-0 glass-card rounded-2xl px-5 py-4 cursor-pointer select-none
        ...
        ${isAnimatingOut && isTop ? 'animate-card-out' : ''}
      `}
      style={style}
      onClick={isTop ? onDismiss : undefined}
      title={isTop ? 'Tap to dismiss' : undefined}
    >
      <div className="flex items-start gap-3">
        ...
        {isTop && (
          <span className="text-[10px] text-[#C4C9D4] font-medium shrink-0 mt-0.5">tap ↑</span>
        )}
      </div>
    </div>
  );
...
export default function StackedActivityFeed({ initialItems = [] }: Props) {
  ...
  const handleDismiss = useCallback(() => {
    if (dismissing || items.length === 0) return;
    setDismissing(true);
    setTimeout(() => {
      setItems((prev) => {
        if (prev.length <= 1) return prev;
        return [...prev.slice(1), prev[0]]; // move top to bottom
      });
      setDismissing(false);
    }, 320);
  }, [dismissing, items.length]);
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-[11px] text-[#C4C9D4] text-center">
        Tap the top card to cycle through activity
      </p>
    </div>
  );
}
